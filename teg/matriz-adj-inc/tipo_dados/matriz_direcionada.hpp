#ifndef _MATRIZ_DIRECIONADA_NAO_INCLUIDO_
#define _MATRIZ_DIRECIONADA_NAO_INCLUIDO_

class Matriz_Direcionada : public Dados { // Matriz é filha de dados
protected:
int qtd_nos;
int qtd_vertices;
...
Matriz_Direcionada(){ // método construtor
	qtd_nos = 0;
	qtd_vertices = 0;
	nome = "Matriz de Adjacencia e Incidencia (Direcionado)";
}

~Matriz_Direcionada(){ //método destrutor
}
...
void mostrarDados(){
	std::cout<<"Matriz de Adjacencia(Direcionada)\n";
	if(!adjacencia.empty()) {
		for(int i = 0; i < qtd_nos; i++) {
			for(int j = 0; j< qtd_nos; j++) {
				if(i <= j )
	...
	std::cout<<"-------------------------\n";
	std::cout<<"Matriz de Incidencia(Direcionada)\n";
	if(!incidencia.empty()) {
